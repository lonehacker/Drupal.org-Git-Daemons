...
from Authenticator.interfaces import IAuthenticator
...
class DrupalorgAuthenticator(object):
    ...
    def getRepositoryAccess(self,argv):
        repostring = argv[-1]
        self.deferred = self.request(repostring)
        self.projectname = self.getprojectname(repostring)
        # Check if pushes are disabled for this path
        self.deferred.addCallback(self.pushcontrol,argv)
        # Once it completes, auth is run
        self.deferred.addCallback(self.auth, argv)
        #self.deferred.addCallback(self.auth_complete)
        return self.deferred
    def auth_complete(self,auth_values):
        self.user,self.auth_service = auth_values
        return None
    def request(self,uri):
        ...
        auth_service.request_json({"project_uri":self.getprojectname(uri)})
    ...
    def auth(self, auth_service, argv):
        ...
        if (not self.user.meta.anonymousReadAccess or \
            ...
            # Username in maintainers list
                ...
                error = "User '{1}' does not have write permissions for repository '{0}'".format(self.projectname, self.user.username)
                return Failure(ConchError(error))
                ...
                # username:password
                    ...
                    error = "Permission denied when accessing '{0}' as user '{1}'".format(self.projectname, self.user.username)
                    return Failure(ConchError(error))
            else:
